...
type Inventario = {
  [ambiente: string]: {
    [mueble: string]: { cantidad: number; observacion: string };
  };
};
...
export default function InventarioForm() {
  ...
  const updateCantidad = (ambiente: string, mueble: string, cantidad: number) => {
    setInventario((prev) => ({
      ...prev,
      [ambiente]: {
        ...prev[ambiente],
        [mueble]: { cantidad, observacion: prev[ambiente]?.[mueble]?.observacion || "" },
      },
    }));
  };

  const updateObservacion = (ambiente: string, mueble: string, observacion: string) => {
    setInventario((prev) => ({
      ...prev,
      [ambiente]: {
        ...prev[ambiente],
        [mueble]: { cantidad: prev[ambiente]?.[mueble]?.cantidad || 0, observacion },
      },
    }));
  };
  ...
  const buildResumen = () => {
    ...
    ambientes.forEach(({ nombre: amb }) => {
      ...
      resumen += `*${amb}*%0A`;
      itemsConCantidad.forEach(([mueble, { cantidad, observacion }]) => {
        resumen += `• ${mueble}: ${cantidad}${observacion ? ` (${observacion})` : ""}%0A`;
      });
      resumen += `%0A`;
    });
        ...
        <div key={amb} className="bg-card border border-border rounded-xl p-6">
          ...
          <div className="space-y-3">
            {muebles.map((mueble) => {
              ...
              return (
                <div key={mueble} className="flex flex-col sm:flex-row sm:items-center gap-2">
                  <span className="font-body text-foreground text-sm w-48 shrink-0">{mueble}</span>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => updateCantidad(amb, mueble, Math.max(0, cantidad - 1))}
                      className="w-8 h-8 rounded-lg border border-border text-foreground hover:border-gold transition-colors font-bold"
                    >
                      −
                    </button>
                    <span className="w-8 text-center font-body text-foreground">{cantidad}</span>
                    <button
                      onClick={() => updateCantidad(amb, mueble, cantidad + 1)}
                      className="w-8 h-8 rounded-lg border border-border text-foreground hover:border-gold transition-colors font-bold"
                    >
                      +
                    </button>
                  ...
                  {cantidad > 0 && (
                    <input
                      type="text"
                      value={observacion}
                      onChange={(e) => updateObservacion(amb, mueble, e.target.value)}
